...
describe('paramify', function(){
  ...
  it('should work with unmapped params', function(done){
    var app = koa();
    var _ = paramify(route);
    ...
    app.use(_.get('/:user/:repo/:file', function*(user, repo, file){
      this.body = this.user + ': ' + this.repo + ': ' + file;
    }));

    request(app.listen())
    .get('/julian/repo/jquery.js')
    .expect('julian: repo: jquery.js', done);
  });
